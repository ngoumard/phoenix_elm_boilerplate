defmodule PhoenixElmBoilerplate.Plug.Locale do
  import Plug.Conn
  ...
  defp handle_locale(conn, %PhoenixElmBoilerplate.User{locale: nil}), do: handle_locale(conn, nil)

  defp handle_locale(conn, %PhoenixElmBoilerplate.User{locale: loc}) do
    assign_locale(conn, loc)
  end

  defp handle_locale(conn, nil) do
    supported_locales = PhoenixElmBoilerplate.Gettext.supported_locales
    locale =
      conn
      |> extract_accept_languages
      |> Enum.filter(&(&1 in supported_locales))
      |> List.first || PhoenixElmBoilerplate.Gettext.config[:default_locale]
  ...
  defp assign_locale(conn, locale) do #Give locale data to all modules that require it
    Gettext.put_locale(PhoenixElmBoilerplate.Gettext, locale)
